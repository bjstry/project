		...
		if(!empty($_GET['type'])){
			...
			if($_GET['type'] == 'steep2'){
				if(empty($_POST['submit'])){
					...
					$cde_extend = "create table if not exists $_POST[dbname].$_POST[dbprefix]de_extend(id int(5) not null auto_increment primary key,did int(1) not null comment '部门id',name varchar(30) not null comment '功能名',url varchar(30) not null comment '功能链接',info varchar(300) not null comment '描述')engine=myisam default charset=utf8";
					$cprjaction = "create table if not exists $_POST[dbname].$_POST[dbprefix]prjaction(id int(5) not null auto_increment primary key,uid int(5) not null comment '用户id',pid int(5) not null comment '项目id',adate varchar(30) not null comment '操作时间',avalue varchar(300) not null comment '操作内容',sdate int(15) not null comment '创建时间',remark varchar(300) not null comment '备注')engine=myisam default charset=utf8";
